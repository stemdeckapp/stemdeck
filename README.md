...
**Duet split.** On a finished job, split the vocals into two singers. Where the two sit in different vocal ranges this is clean. Where they share a range, a second model handles it and puts the other singer roughly 14 dB down, which is a real split rather than an isolation. A chorus is beyond both, and `docs/models.md` has the measurements behind all of that.
